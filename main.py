...
def check_removed(card_clicked):
    if len(selected_card
           ) > 0 and selected_card.sprite.rank + card_clicked.rank == 13:
...
def main():
    ...
    while True:
        ...
        if len(selected_card) == 1:
            pygame.draw.rect(screen, (204, 173, 0), selected_card.sprite.rect,
                             3)
        if len(next_card) == 0:
        ...
        if len(board) == 0:
          ...
          if len(Y) > 1:
            while len(Y) > 1:
              Y.pop()
          win_text = font.render("You Win in {} seconds!".format(int(*Y)/100),True,(255,255,255))
